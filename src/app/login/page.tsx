...
import { createClient, isSupabaseConfigured, isDemoModeAllowed } from "@/lib/supabase/client";
import { shouldShowDemoEntry } from "@/lib/login-demo-entry";

// Supabase未設定（本番URL/キー未設定のプレビュー環境等）かつデモモードが明示的に禁止
// （NEXT_PUBLIC_ALLOW_DEMO=false）されていない場合のみ、デモ版への入口を表示する。
// Supabase設定済みの本番環境では、この値は常にfalseになりGoogleログインのみが表示される。
const SHOW_DEMO_ENTRY = shouldShowDemoEntry(isSupabaseConfigured, isDemoModeAllowed);
