...
import { companySafetyErrorMessage } from "@/lib/company-safety";
import {
  ...
  return <div className="min-h-screen bg-[#f5f7fb] text-[#172036]">
    ...
    <main className="min-h-screen lg:ml-[238px]">
      ...
      <div className="px-4 pb-10 md:px-8">{loading?<Loading/>:<>
        ...
        {view==="call"&&<CallScreen company={callableCompanies[callIndex]} hasBlockedRemaining={callableCompanies.length===0&&filteredCompanies.length>0} member={currentUserName} next={()=>setCallIndex(i=>callableCompanies.length?(i+1)%callableCompanies.length:0)} onSaved={refresh} notify={notify}/>}
    ...
    {importModal&&<LeadImportModal member={currentUserName} existing={companies} close={()=>setImportModal(false)} saved={async s=>{await refresh();setImportModal(false);notify(`${s.inserted}件の営業先を取り込みました${s.skipped||s.blocked?`（重複${s.skipped}件・架電禁止${s.blocked}件はスキップ）`:""}`)}}/>}
    {csvModal&&<CsvImportModal member={currentUserName} existing={companies} close={()=>setCsvModal(false)} saved={async summary=>{await refresh();setCsvModal(false);notify(`新規${summary.newCount}件・更新${summary.updatedCount}件を登録しました（スキップ${summary.skippedCount}件・架電禁止${summary.blockedCount}件・エラー${summary.errorCount}件）`)}}/>}
...
function BlockModal({company,close,done,notify}:{company:Company;close:()=>void;done:()=>Promise<void>;notify:(s:string)=>void}) {
 ...
 const submit=async()=>{
  ...
  try{
   if(isUnblock){
    ...
    await blockCompanyCalls(company.id,reason);
   }
   await done();
 ...
 return <Modal title={isUnblock?"架電禁止を解除":"架電禁止に設定"} sub={company.name} close={close}>
  {isUnblock&&company.blocked_reason&&<p className="mb-3 rounded-lg bg-red-50 p-3 text-xs text-red-700">現在の禁止理由：{company.blocked_reason}</p>}
...
function LeadImportModal({member,existing,close,saved}:{member:string;existing:Company[];close:()=>void;saved:(summary:{inserted:number;skipped:number;blocked:number})=>void}) {const [raw,setRaw]=useState("株式会社サンプル商事,076-000-0000,https://example.com\n石川採用テック 076-111-2222 www.example.jp");const [defaults,setDefaults]=useState({industry:"",location:"石川県",source:""});const [busy,setBusy]=useState(false);const leads=useMemo(()=>parseLeadRows(raw,member,existing,defaults),[raw,member,existing,defaults]);
...
function CsvImportModal({member,existing,close,saved}:{member:string;existing:Company[];close:()=>void;saved:(s:{newCount:number;updatedCount:number;skippedCount:number;blockedCount:number;errorCount:number})=>void}) {
 ...
 const submit=async()=>{
  ...
  try{
   ...
   const errorRows:CsvRowResult[]=[
    ...
    ...results.filter(r=>r.status==="error").map(r=>({...eligible[r.row-1],errors:[r.error_message?companySafetyErrorMessage(r.error_message):"登録に失敗しました"]})),
   ];
