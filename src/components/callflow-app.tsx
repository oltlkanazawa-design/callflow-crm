...
function CallScreen({company,hasBlockedRemaining,member,next,onSaved,notify,recordingEnabled,recordingLocked,onRecordingLockChange,recorderRef}:{company?:Company;hasBlockedRemaining?:boolean;member:string;next:()=>void;onSaved:()=>Promise<void>;notify:(s:string)=>void;recordingEnabled:boolean;recordingLocked:boolean;onRecordingLockChange:(lock:CallRecorderLockState)=>void;recorderRef:React.RefObject<CallRecorderHandle|null>}) { const [staged]=useState<(TranscriptAnalysis&{transcript?:string})|null>(()=>{if(typeof window==="undefined")return null;const raw=sessionStorage.getItem("callflow_analysis");if(!raw)return null;sessionStorage.removeItem("callflow_analysis");try{return JSON.parse(raw) as TranscriptAnalysis&{transcript?:string}}catch{return null}});const [result,setResult]=useState<CallResult|"">(staged?.result||"");const [note,setNote]=useState(staged?`${staged.summary}\n次回対応：${staged.next_action}`:"");const [nextAt,setNextAt]=useState(staged?.next_action_at||"");const [saving,setSaving]=useState(false);
 ...
 const applyAnalysis=(fields:AppliedAnalysisFields)=>{
  ...
  setNote(parts.join("\n"));
  // <input type="datetime-local">はタイムゾーンオフセット付きISO文字列（Codexの出力形式）を
  // そのままでは表示できない（空欄になる）ため、ローカル時刻のYYYY-MM-DDTHH:mmへ変換する。
  setNextAt(toDatetimeLocalValue(fields.nextActionAt));
 ...
 const save=async()=>{if(!result)return notify("架電結果を選択してください");setSaving(true);const created=new Date().toISOString(),updated={...company,heat:appliedAnalysis?.applyHeat?appliedAnalysis.heat as Heat:(result==="アポ獲得"?"高" as Heat:company.heat),memo:note||company.memo,last_called_at:created,next_action_at:nextAt||undefined};const log:CallLog={id:crypto.randomUUID(),company_id:company.id,company_name:company.name,caller_name:member,result,note:note||"メモなし",transcript:appliedAnalysis?.transcript??staged?.transcript,ai_summary:appliedAnalysis?.summary??staged?.summary,created_at:created,next_action_at:nextAt||undefined};try{await saveCallLog(log,updated);await onSaved();notify("架電記録を保存しました");setResult("");setNote("");setNextAt("");setAppliedAnalysis(null);next()}catch(e){notify(e instanceof Error?e.message:"保存に失敗しました")}finally{setSaving(false)}};
...
/** タイムゾーンオフセット付きISO文字列（Codex解析結果のnext_action_at等）を<input type="datetime-local">用のYYYY-MM-DDTHH:mm（ローカル時刻）へ変換する。 */
