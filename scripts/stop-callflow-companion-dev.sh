#!/bin/bash
# CallFlow Companion（開発用HTTPモード）を安全に停止する。
# 対象PIDが本当にCallFlow Companionのプロセスであることを確認してから停止する。
# killall/pkillは使用しない（無関係なプロセスを巻き込まないため）。
set -euo pipefail
...
# 対象PIDが本当にCallFlow Companionのプロセスかを確認する（PID再利用対策）。
...
# サーバー自身の終了処理でも一時ファイルは削除されるが、念のため残存物を清掃する。
if [ -d "$TMP_DIR" ]; then
