...
-- dblink_dbname / dblink_user / dblink_password を -v で渡すこと
-- （現在接続中の資格情報と同じものを渡す。推測のパスワードに依存しないため）。
-- 接続はhost/portを指定せず既定のUNIXソケット経由で行う。dblinkはPostgres
-- サーバー自身の内部から新規接続するため、クライアント側から見えるDocker/CLIの
-- ポート転送（例：supabase CLIの54322）はサーバー内部からは使えないことに注意。
-- 例：
--   psql "$DB_URL" -v dblink_dbname=... -v dblink_user=... \
--        -v dblink_password=... -f company-safety-concurrency.sql
-- =========================================================
