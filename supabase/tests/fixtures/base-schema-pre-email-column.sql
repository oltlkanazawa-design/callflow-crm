-- =========================================================
-- テスト専用フィクスチャ：companiesへのemail列追加（add-company-email-column.sql）
-- 着手前のschema.sql相当。
--
-- 「本番同様に、companiesがemail列を持たない状態で作成され、後から
-- ALTER TABLE ADD COLUMN emailで追加された」という履歴を、CI上で正確に
-- 再現するための基準スキーマ。base-schema-pre-company-safety.sqlとの違いは
-- companiesテーブル定義にemail列を含めていない点のみ。
--
-- 用途：company-detail-edit-archive-core.sqlのT20（companies_with_call_status
-- ビューの列順序が、本番の実際の物理列順「…, updated_at, email」と一致する
-- ことを検証するテスト）を、本番と同じ履歴のデータベースに対して実行するため
-- （新規schema.sqlだけから作った環境ではemailがlist_sourceの直後に来るため、
-- この不一致を検出できない）。
--
...
-- 【重要】email列を意図的に含めない（本番でALTER TABLEにより後から追加された
-- 経緯を再現するため）
